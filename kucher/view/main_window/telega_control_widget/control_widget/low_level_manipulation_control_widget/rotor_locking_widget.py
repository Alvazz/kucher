...
class Widget(LowLevelManipulationControlSubWidgetBase):
    ...
    def __init__(self,
        ...
        self._current_control.spinbox_suffix = 'Ampere'
        self._current_control.num_decimals = 3
        self._current_control.tool_tip = 'Current setpoint, Ampere'
        self._current_control.status_tip = self._current_control.tool_tip
        self._current_control.slider_visible = False

        self._target_angvel_control =\
            SpinboxLinkedWithSlider(self,
                                    minimum=-99.0,
                                    maximum=99.0,
                                    step=5.0,
                                    slider_orientation=SpinboxLinkedWithSlider.SliderOrientation.HORIZONTAL)
        self._target_angvel_control.spinbox_suffix = 'rad/sec '
        self._target_angvel_control.tool_tip = 'Target angular velocity, rad/sec'
        self._target_angvel_control.status_tip = self._target_angvel_control.tool_tip

        # Initial values
        self._current_control.value     = 0.1
        self._target_angvel_control.value   = 5.0

        # Having configured the values, connecting the events
        self._current_control.value_change_event.connect(self._on_any_control_changed)
        self._target_angvel_control.value_change_event.connect(self._on_any_control_changed)

        self.setLayout(
            lay_out_horizontally(
                self._current_control.spinbox,
                self._target_angvel_control.spinbox,
                make_button(self,
                            icon_name='clear-symbol',
                            tool_tip='Reset the target angvelocity to zero',
                            on_clicked=self._on_target_angvel_clear_button_clicked),
                self._target_angvel_control.slider,
                self._send_button,
            )
        )

    def get_widget_name_and_icon_name(self):
        return 'Rotor locking', 'angvel'
    ...
    def stop(self):
        ...
        self._target_angvel_control.value = 0
        self._current_control.value = 0
        ...
        with self._with_events_suppressed():    # Paranoia
            if self._send_button.isChecked():
                vector = [
                    ...
                    self._target_angvel_control.value,
                ]
                _logger.debug('Sending rotor locking command %r', vector)
                self._launch_async(self._commander.low_level_manipulate(LowLevelManipulationMode.ROTOR_LOCKING,
                                                                        *vector))
    ...
    def _on_target_angvel_clear_button_clicked(self):
        self._target_angvel_control.value = 0
